...
class GovernanceClass(object):
    ...
    def vote(self, galactrumd, signal, outcome):
        return self.go.vote(galactrumd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, galactrumd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(galactrumd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(galactrumd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to galactrumd "gobject list" output
    ...
    def galactrumd_serialise(self):
        import galactrumlib
        return galactrumlib.SHIM_serialise_for_galactrumd(self.serialise())
