...
def is_valid_galactrum_address(address, network='mainnet'):
    ...
    galactrum_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Galactrum address
        return False

    if (address_version != galactrum_version):
        return False
...
# shims 'til we can fix the galactrumd side
def SHIM_serialise_for_galactrumd(sentinel_hex):
    ...
    # superblock => "trigger" in galactrumd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # galactrumd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    galactrumd_hex = serialise(obj)
    return galactrumd_hex


# shims 'til we can fix the galactrumd side
def SHIM_deserialise_from_galactrumd(galactrumd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(galactrumd_hex)

    # shim from galactrumd
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('galactrum.conf')
