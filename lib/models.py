...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, galactrumd):
        golist = galactrumd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_galactrumd(galactrumd, item)
    ...
    @classmethod
    def import_gobject_from_galactrumd(self, galactrumd, rec):
        import decimal
        import galactrumlib
        import inflection
        ...
        # shim/galactrumd conversion
        object_hex = galactrumlib.SHIM_deserialise_from_galactrumd(object_hex)
        objects = galactrumlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from galactrumd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from galactrumd, with every run
        ...
        # get/create, then sync payment amounts, etc. from galactrumd - Galactrumd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(galactrumd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from galactrumd! %s" % e)
            govobj.vote_delete(galactrumd)
            return (govobj, None)
    ...
    def vote_delete(self, galactrumd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(galactrumd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, galactrumd, signal, outcome):
        import galactrumlib
        ...
        output = galactrumd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = galactrumlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(galactrumd, signal)

    def sync_network_vote(self, galactrumd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = galactrumd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 galactrum addr, non-multisig
            if not galactrumlib.is_valid_galactrum_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Galactrum address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (galactrumlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/GalactrumDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import galactrumlib
        obj_data = galactrumlib.SHIM_serialise_for_galactrumd(self.serialise())
    ...
    def prepare(self, galactrumd):
        try:
            object_hash = galactrumd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not galactrumlib.is_valid_galactrum_address(addr, config.network):
    ...
    def hash(self):
        import galactrumlib
        return galactrumlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, galactrumd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - galactrumd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, galactrumd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - galactrumd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, galactrumd):
        now = int(time.time())
        return (self.created_at < (now - galactrumd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, galactrumd):
        if self.is_expired(galactrumd):
            return False

        return True

    def is_deletable(self, galactrumd):
        if self.is_expired(galactrumd):
            return True
